...
START_TOKEN = 0
PRE_EPOCH_NUM = 40 # supervise (maximum likelihood estimation) epochs
SEED = 44
BATCH_SIZE = 9
LEARNING_RATE = 0.01
GOAL_SIZE = 8
...
def robust_loss(sess,sentence,data_loader):
    # robust_loss means the oracle robustness degree measure with sentence
    nll =[]
    data_loader.reset_pointer()
...
def get_reward(model,dis, sess, input_x, rollout_num, dis_dropout_keep_prob,vocab,params):
    rewards = []
...
def main():
    ...
    vocab_size = 1000
    vocab =   generate_vocab(vocab_size,Name_Length,LOW_TIME,HIGH_TIME,0,1)  #geneate vocab_size words   vocab_size x word_dim
    ...
    sslgan = SSLGAN(SEQ_LENGTH,num_classes=2,vocab_size=vocab_size,emb_dim=EMB_DIM,dis_emb_dim=dis_embedding_dim,filter_sizes=dis_filter_sizes,num_filters=dis_num_filters,
    ...
    #target_robust = TARGET_ROBUST(sequence_token, vocab, params)  # The oracle model
    ...
    if FLAGS.restore and model:
        ...
        if FLAGS.resD and model_path + '/' + FLAGS.model:
                ...
                for i in range(10):
                    for _ in range(5):
                        generate_samples(sess, sslgan, BATCH_SIZE, generated_num, negative_file,0)
                        generate_samples(sess, target_lstm, BATCH_SIZE, generated_num, positive_file,0)
                        # gen_data_loader.create_batches(positive_file)
    ...
    for total_batch in range(TOTAL_BATCH):
        ...
        for it in range(1):
            ...
            for gi in range(gencircle):
                ...
                _,_,g_loss,w_loss = sess.run([sslgan.manager_updates,sslgan.worker_updates,sslgan.goal_loss,sslgan.worker_loss], feed_dict=feed)
